      ...
      <div class="bg-gray-900 rounded-b-xl overflow-hidden">
        <div class="preview-ratio">
          <div class="preview-content">
            <iframe v-if="liveStreamSrc" :key="iframeKey" :src="liveStreamSrc" class="embed-frame" frameborder="0"
              scrolling="no" allow="autoplay; fullscreen; picture-in-picture; encrypted-media" allowfullscreen
              referrerpolicy="origin-when-cross-origin" title="Live stream embed"></iframe>
    ...
    <div class="card bg-white rounded-xl shadow-2xl p-6">
      ...
      <form class="card_rec" @submit.prevent="handleLivestreamForm">
        <div class="form-group mb-6 mt-4">
          <h4 for="streamerNameInput">Streamer Name:
            <input type="text" id="streamerNameInput" v-model.trim="streamerName"
              class="form-control col-span-1 md:col-span-1 p-2 border rounded-md" placeholder="e.g., JaneDoe123"
              required />
          </h4>
        </div>
        ...
        <div class="options-group mb-8">
          ...
          <div class="options-container">
            <div v-for="option in streamOptions" :key="option.value" class="flex items-center gap-2">
              <input class="form-radio" type="radio" :id="`option-${option.value}`" :value="option.value"
                v-model="selectedOption" />
              <label :for="`option-${option.value}`" class="cursor-pointer select-none"
